Models
	...
	Executives: User Awway required default empty → List of users that are executives
	...
	comments: List default [] -> list of all comments made on that post
...
/clubs/addExecutive
        	    http post request that takes a JSON body containing the username username adds that user to the Executive list

/clubs/:clubName/Posts
        	    http get request that gets all posts related to that clubName

/clubs/:clubName/:executive
	Http get request that returns 0 or 1 if the executive provided is inside the executives list for that club
...
/clubs/join/:id
	...
	/users/getUser/:username
	     http get request returns user information from the database given the username
